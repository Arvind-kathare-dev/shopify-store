    ...
    <section className="w-full py-12 md:py-24 px-4 sm:px-6 md:px-12 lg:px-20 bg-white">
      <div className="max-w-[1440px] mx-auto">
        ...
        >
          ...
          >
            <div className="flex-[1.5] text-white max-w-[650px] text-center lg:text-left">
            ...
            <div className="flex-1 flex flex-col gap-4 w-full max-w-[465px]">
              {[
                ...
                <div key={i} className="bg-white/10 backdrop-blur-md flex flex-col gap-2 rounded-2xl p-5 md:p-6 border border-white/20">
          ...
          <div className="p-6 md:p-12 lg:p-16">
              ...
              {timelineData.map((item, index) => (
                ...
                >
                  ...
                  {index !== timelineData.length - 1 && (
                    <div className="absolute left-8 md:left-[45px] top-8 md:top-[45px] bottom-0 w-px border-l-2 border-dotted border-[#EBE8F4] z-0" />
                  )}

                  <div className="flex-shrink-0 w-16 h-16 md:w-[90px] md:h-[90px] rounded-full bg-[#F5F2FF] flex items-center justify-center border-4 border-white shadow-sm transition-transform hover:scale-110 relative z-10">
                    <Image
                      src={item.iconSrc}
                      alt="step icon"
                      width={34}
                  ...
                  <div className="pt-1 md:pt-3 relative z-10">
                    <span className="text-secondary font-bold text-[10px] md:text-xs uppercase tracking-[0.15em] mb-2 block">
                      {item.day}
                    </span>
                    <h3 className="text-lg sm:text-xl md:text-[24px] font-bold text-[#1A1A1A] mb-2 md:mb-3 leading-snug">
                      {item.title}
                    </h3>
                    <p className="text-sm md:text-[16px] text-[#666] leading-relaxed max-w-auto font-normal">
                      {item.description}
                    </p>
                  </div>
                </motion.div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-16 md:mt-24">
              {statCards.map((stat, i) => (
                <div
                  key={i}
                  className="bg-[#F9F8FF] p-6 md:p-8 rounded-[24px] text-center border border-[#F1EEFF] hover:border-secondary/20 transition-all group"
                >
                  <span className="text-base sm:text-lg md:text-xl font-bold text-secondary block mb-2 group-hover:scale-105 transition-transform">{stat.value}</span>
        ...
        >
          <div className="flex flex-col md:flex-row items-center md:items-start text-center md:text-left gap-6">
            <div className="w-16 h-16 rounded-2xl bg-white flex items-center justify-center shadow-[0_10px_25px_-5px_rgba(230,168,0,0.1)] flex-shrink-0">
